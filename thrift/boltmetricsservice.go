...
func NewBoltMetricsServiceProcessor(handler BoltMetricsService) *BoltMetricsServiceProcessor {
	...
	self122 := &BoltMetricsServiceProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self122.processorMap["gauge"] = &boltMetricsServiceProcessorGauge{handler: handler}
	self122.processorMap["timer"] = &boltMetricsServiceProcessorTimer{handler: handler}
	self122.processorMap["histogram"] = &boltMetricsServiceProcessorHistogram{handler: handler}
	self122.processorMap["sum"] = &boltMetricsServiceProcessorSum{handler: handler}
	return self122
}
