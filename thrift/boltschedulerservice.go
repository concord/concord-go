...
func NewBoltSchedulerServiceProcessor(handler BoltSchedulerService) *BoltSchedulerServiceProcessor {
	...
	self100 := &BoltSchedulerServiceProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self100.processorMap["deployComputation"] = &boltSchedulerServiceProcessorDeployComputation{handler: handler}
	self100.processorMap["getComputationSlug"] = &boltSchedulerServiceProcessorGetComputationSlug{handler: handler}
	self100.processorMap["registerComputation"] = &boltSchedulerServiceProcessorRegisterComputation{handler: handler}
	self100.processorMap["scaleComputation"] = &boltSchedulerServiceProcessorScaleComputation{handler: handler}
	self100.processorMap["killTask"] = &boltSchedulerServiceProcessorKillTask{handler: handler}
	return self100
}
...
func (p *boltSchedulerServiceProcessorGetComputationSlug) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	var retval []byte
	var err2 error
	if retval, err2 = p.handler.GetComputationSlug(args.ComputationName); err2 != nil {
		switch v := err2.(type) {
		case *BoltError:
			result.E = v
		default:
...
func (p *boltSchedulerServiceProcessorRegisterComputation) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	iprot.ReadMessageEnd()
	result := RegisterComputationResult{}
	var retval *TopologyMetadata
	var err2 error
	if retval, err2 = p.handler.RegisterComputation(args.Computation); err2 != nil {
		switch v := err2.(type) {
		case *BoltError:
			result.E = v
		default:
